...
.disclaimer{margin:28px 0 8px;padding-top:14px;border-top:1px solid var(--bd);color:var(--mut);font-size:12px;line-height:1.65}
.disclaimer b{color:var(--txt)}.disclaimer a{color:var(--red);text-decoration:none}.disclaimer a:hover{text-decoration:underline}
...
<small class=note>Scores every config on the eval set's retrieval — no LLM calls — and selects the winner. <b>This is what unlocks the “AutoRAG winner” everywhere else.</b> <i>Demo scope: a small grid + deterministic metric — see the note at the bottom of the page.</i></small></div>
...
<b>Minimal demo.</b> This runs the <b>real</b> AutoRAG loop — a grid sweep over chunk size · overlap · top-k · retriever,
scored on a local eval set, with answers generated by a local 1B model — but only a small slice of it. A production /
full open-source AutoRAG also sweeps embedding models, rerankers, query expansion and prompt templates, scores with
LLM-judge metrics (faithfulness, answer correctness, cost/latency), and uses a real vector DB. Here the search space is
small, the metric is deterministic gold-phrase retrieval, and the store is in-memory — enough to show the method honestly,
not the full product. See <code>autorag/README.md</code> and <code>ADOPT.md</code> for the full picture.
</footer>
</div>
<script>
